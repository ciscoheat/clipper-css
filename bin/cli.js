...
async function main() {
    ...
    const isAstro = await exists(path.join(cwd, 'astro.config.mjs')) || 
                   await exists(path.join(cwd, 'astro.config.ts'));
    
    const isSvelte = await exists(path.join(cwd, 'svelte.config.js')) || 
                     await exists(path.join(cwd, 'svelte.config.ts'));

    const isNext = await exists(path.join(cwd, 'next.config.js')) || 
    ...
    const clipperSource = path.join(templatesDir, 'base', 'src', 'clipper');
    ...
    if (type === 'astro') {
        ...
        console.log('   Import "src/lib/clipper/clipper.css" in your +layout.svelte.');
    }
}
