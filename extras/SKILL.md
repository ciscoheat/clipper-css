...
Treat an existing route as contaminated demo content until proven otherwise. Before implementation, remove or replace its visible page markup, demo data, page-specific styles, and imported demo components. Retain only framework setup, routing, and source-faithful code. Do not use the existing page as a visual starting point or a source of section labels.
...
Use template utilities and local page styles for source-specific layout and typography. Do not apply a single centered container, `readable` width, or default section rhythm to every section: implement the alignment and width recorded in the geometry map. Update `components.css` for generic primitives that recur in the source; replace existing primitives when their current styling conflicts with the source. Keep truly one-off sections composed in their template.
