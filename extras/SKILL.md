...
- Do not use `max-w-*`, `w-*`, `w-fit`, `readable`, or `mx-auto` on text, headings, article bodies, or their wrappers as a generic readability or composition device. Omit width constraints unless the source visibly places that exact text block in a bounded column; when it does, record the source width in the geometry map and use one specific matching static utility or arbitrary value. No recorded width means no width utility.
...
Do not add `lg:` or any responsive breakpoint as a generic desktop default. Use the breakpoint map derived from the supplied reference widths. A content group that spans its parent in the source must remain `w-full` and must not be narrowed by an unrecorded descendant width constraint.
