...
For live pages, perform a DOM asset audit in addition to the accessibility snapshot. Enumerate every repeated peer group by selector or landmark, including links with empty accessible names, image-only links, inline SVG/icon links, CSS-background images, carousels, and cards below the fold. Capture each item's href, text or aria-label, media source or SVG presence, and bounding box. A heading plus supporting sentence is not a complete section when the source contains a repeated group beneath it. Do not mark mapping complete until the rendered peer count and source order are recorded, and every card or item has an evidence entry.

Use the supplied widths to map responsive states and only the transitions the evidence supports. A centered parent is not proof of centered text: map the text block's own left and right edges. **Done when** every visible band, peer group, text role, and component has a map entry, repeated counts agree with the DOM audit, and no below-the-fold or media-only group remains unaccounted for.
...
Declare every mapped row with `row` and every mapped grid with explicit columns. Use `full-width` for a full-bleed hero background and a `page-width` child for its bounded content. Size peer components with shared explicit tracks or `w-full`; their content, images, borders, and wrappers must not alter equal source widths. For centered text, give the block its mapped width before centering text. Use `min-w-0` where text must shrink. Before considering a section complete, compare its mapped repeated-item count with the source DOM and screenshot. If a source card has no text node, preserve it through its link, media, SVG, CSS background, or computed bounding box rather than dropping it as empty. **Done when** the route expresses the evidence map without invented sections, decoration, or omitted repeated items.
...
Inspect computed styles for each band background, representative foreground and border, and each text role's `font-family`, `font-size`, `font-weight`, `line-height`, and `letter-spacing`. Inspect text bounding boxes for true centering and repeated component outer boxes for intended equal widths. Check focus, overflow, and wrapping; text must retain source-supported width without isolated short lines. Re-query the rendered DOM and assert that every mapped repeated group has the same item count, item order, destinations, and media presence as the source. Rework the evidence map, tokens, or utilities until screenshots match at a glance.
